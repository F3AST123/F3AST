...
from train_vid2seq import F3AST, evaluate
...
def get_best_epoch(model_dir, key='val_acc'):
...
def main(model_dir, frame_dir, split, dataset):
    ...
    model = F3AST(len(classes) + 1, config['feature_arch'], config['temporal_arch'], clip_len=config['clip_len'],
                  step=config['stride'], multi_gpu=config['gpu_parallel'], use_local=config['use_local'],
                  sparse_att_mask=config['sparse_att_mask'], max_seq_len=config['max_seq_len'])

    model.load(torch.load(os.path.join(
        model_dir, 'checkpoint_{:03d}.pt'.format(best_epoch))))

    split_path = os.path.join('data', dataset, '{}.json'.format(split))
    split_data = ActionSeqVideoDataset(classes, split_path, frame_dir, config['clip_len'], overlap_len=0,
                                       local_frame_dir=config['local_frame_dir'], max_seq_len=config['max_seq_len'],
                                       crop_dim=config['crop_dim'], stride=config['stride'])

    evaluate(model, split_data, split.upper(), classes, with_tolerance=config['with_tolerance'])
