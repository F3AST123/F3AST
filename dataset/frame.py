...
class FrameReader:
    ...
    def __init__(self, frame_dir, crop_transform, img_transform, same_transform, local_frame_dir=None):
        self._frame_dir = frame_dir
        self._local_frame_dir = local_frame_dir
    ...
    def load_frames(self, video_name, start, end, pad=False, stride=1,
        ...
        for frame_num in range(start, end, stride):
            ...
            if self._local_frame_dir is not None:
                local_frame_path = os.path.join(self._local_frame_dir, video_name, img_num)
            try:
                if self._frame_dir is not None:
                    img = self.read_frame(frame_path)
                if self._local_frame_dir is not None:
                    local_img = self.read_frame(local_frame_path)
                if self._crop_transform:
                    ...
                    if self._crop_transform is not None:
                        ...
                        if self._local_frame_dir is not None:
                            local_img = self._crop_transform(local_img)
                ...
                if not self._same_transform:
                    ...
                    if self._local_frame_dir is not None:
                        local_img = self._img_transform(local_img)
                if self._frame_dir is not None:
                    ret.append(img)
                if self._local_frame_dir is not None:
                    ret.append(local_img)
            except RuntimeError:
        ...
        if self._local_frame_dir is not None and self._frame_dir is not None:
            idx = [i for i in range(ret.shape[0]) if i % 2 == 0]
            lcl_idx = [i for i in range(ret.shape[0]) if i % 2 == 1]
            ret_lcl = ret[lcl_idx]
            ret = ret[idx]
        elif self._local_frame_dir is not None and self._frame_dir is None:
            ret_lcl = copy.deepcopy(ret)
        ...
        if n_pad_start > 0 or (pad and n_pad_end > 0):
            ...
            if self._local_frame_dir is not None:
                ret_lcl = nn.functional.pad(
                    ret_lcl, (0, 0, 0, 0, 0, 0, n_pad_start, n_pad_end if pad else 0))
...
def _load_frame_deferred(gpu_transform, batch, device):
    ...
    local_frame = batch['local_frame'].to(device)
    with torch.no_grad():
        for i in range(frame.shape[0]):
            frame[i] = gpu_transform(frame[i])
        for i in range(local_frame.shape[0]):
            local_frame[i] = gpu_transform(local_frame[i])

    return frame, local_frame
...
class ActionSeqDataset(Dataset):
    ...
    def load_frame_gpu(self, batch, device):
        ...
        if self._gpu_transform is None:
            ...
            frame, local_frame = _load_frame_deferred(self._gpu_transform, batch, device)
        return frame, local_frame, key_padding_mask.to(device)
    ...
    def _get_one(self):
        ...
        # sequence label
        labels = np.zeros(self._max_seq_len, int)
        i = 0
        label_id_name = {}
        labels[i] = 1  # start
        labels[i + 1] = 2  # end
        for event in video_meta['events']:
            ...
            if event_frame >= base_idx and event_frame < base_idx + self._clip_len * stride:
                labels[i + 1] = self._class_dict[event['label']]
                label_id_name[self._class_dict[event['label']]] = event['label']
                labels[i + 2] = 2  # end
                i += 1

        frames, local_frame = self._frame_reader.load_frames(
...
class ActionSeqVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'video': video_name, 'fps': fps, 'start': start // stride,
                'frame': frames, 'local_frame': local_frame,
    ...
    def get_labels_start(self, video, start):
        meta = self._labels[self._video_idxs[video]]
        end = start + self._clip_len
        num_frames = meta['num_frames']
        num_labels = num_frames // self._stride
        labels = np.zeros(num_labels, int)
        for event in meta['events']:
            frame = event['frame']
            if frame // self._stride > start and frame // self._stride < end:
                labels[frame // self._stride] = self._class_dict[event['label']]
        return labels
